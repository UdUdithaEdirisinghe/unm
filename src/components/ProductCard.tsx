...
export default function ProductCard({ product }: Props) {
  ...
  const addToCart = () => {
    add({ id: String(product.id), name: s(product.name) || "Product", price: priceToUse, image: img, slug: s(product.slug) }, 1);
    toast.success(`${s(product.name) || "Product"} added to cart!`);
  };
  ...
  return (
    <div className="relative flex flex-col rounded-lg border border-slate-800/60 bg-[rgba(10,15,28,0.6)] p-3">
      ...
      <div className="absolute top-3 left-3 z-10 flex flex-col gap-2">
        {out && <span className="rounded-md bg-rose-600 px-2 py-1 text-xs font-semibold text-white shadow">Out of stock</span>}
        {!out && pct > 0 && <span className="rounded-md bg-emerald-600 px-2 py-1 text-xs font-semibold text-white shadow">-{pct}%</span>}
      </div>

      {/* image */}
      <Link href={`/products/${s(product.slug)}`} className="block rounded-md overflow-hidden border border-slate-800/60 bg-[rgba(10,15,28,0.4)]">
      ...
      <div className="mt-3 flex-1 min-h-[3.25rem]">
        ...
        {s(product.brand) && <div className="text-xs text-slate-400 truncate">{s(product.brand)}</div>}
        {s((product as any).category) && <div className="text-xs text-slate-500 truncate">{s((product as any).category)}</div>}
      </div>
      ...
      <div className="mt-2 flex flex-col sm:flex-row sm:items-baseline sm:gap-2">
        ...
        {hasSale && <span className="text-sm text-slate-400 line-through">{formatCurrency(price)}</span>}
      </div>

      {/* button */}
      <div className="mt-3">
        <button className={`btn-primary w-full ${out ? "opacity-50 cursor-not-allowed" : ""}`} disabled={out} onClick={addToCart}>
