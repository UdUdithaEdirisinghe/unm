...
const {
  ...
  SMTP_USER,          // support@manny.lk  (Zoho user or SMTP-enabled alias)
  SMTP_PASS,          // Zoho "App Password"
  MAIL_FROM,          // 'Manny.lk <support@manny.lk>'  (should match/alias SMTP_USER)
  MAIL_TO_ORDERS,     // orders@manny.lk
} = process.env;
...
async function getTransporter(): Promise<nodemailer.Transporter> {
  ...
  const useSSL = port === 465; // Zoho: 465 = SSL, 587 = STARTTLS

  const transporter = nodemailer.createTransport({
    host,
    port,
    secure: useSSL,
    requireTLS: !useSSL,          // force STARTTLS on 587
    ...
    tls: useSSL ? undefined : { rejectUnauthorized: true },
  });

  try {
    console.log(`[mail] verifying SMTP ${host}:${port} secure=${useSSL}`);
    await transporter.verify();
    console.log("[mail] SMTP verify OK");
  } catch (err: any) {
    console.error("[mail] SMTP verify FAILED:", err?.message || err);
    // We still return the transporter so order flow isn’t blocked;
    // sendMail will log errors if it fails.
  }
...
async function send(opts: SendMailOptions) {
  ...
  console.log("[mail] sent", { id: info.messageId, to: opts.to, subject: opts.subject, resp: info.response });
  return info;
}
...
export async function sendOrderEmails(order: OrderEmail) {
  const fromAddr = MAIL_FROM || `Manny.lk <${SMTP_USER}>`; // must match SMTP_USER or an SMTP-enabled alias
  ...
  try {
    await send({
      ...
      replyTo: MAIL_TO_ORDERS || SMTP_USER, // replies come to orders/support
    });
  } catch (e: any) {
    console.error("[mail] customer email failed:", e?.message || e);
  }
  ...
  try {
    ...
    console.error("[mail] admin email failed:", e?.message || e);
  }
}