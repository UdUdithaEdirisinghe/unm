...
function getTransporter() {
  ...
  transporter = nodemailer.createTransport({
    ...
    secure: false, // <-- 587 uses STARTTLS, so this MUST be false
...
type OrderEmail = {
  id: string;
  createdAt: string;
  customer: { firstName: string; lastName: string; email: string; phone?: string; address: string; city: string; postal?: string; notes?: string };
  items: Line[];
  subtotal: number;
  shipping: number;
  total: number;
...
export async function sendOrderEmails(order: OrderEmail) {
  // Fail-safe: never throw from here; don’t block order creation
  try {
