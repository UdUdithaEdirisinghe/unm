  ...
  async function load() {
    ...
    try {
      const [pr, pm, or, sc] = await Promise.all([
        ...
        safeGetArray<Order>(
          orderFilter === "all" ? "/api/orders" : `/api/orders?status=${orderFilter}`
        ),
    ...
    <div className="mx-auto w-full max-w-5xl px-4 py-6">
      ...
      >
        ...
        <div className="md:col-span-5 flex items-center justify-end gap-2">
          <button type="button" className="btn-ghost" onClick={resetCredit}>
            New
          </button>
          <button type="submit" className="btn-primary" disabled={cSaving}>
            {cSaving ? "Saving…" : "Save store credit"}
          </button>
        </div>
      </form>
      ...
      <div className="mt-4 rounded-xl border border-slate-800/60 bg-[rgba(10,15,28,0.6)] p-4">
        {credits.length === 0 ? (
          <p className="text-sm text-slate-400">No store credits.</p>
        ) : (
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            {credits.map((sc) => (
              <div
