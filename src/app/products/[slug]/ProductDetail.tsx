...
/* ---------- Related (Featured-first, fallback to same category) ---------- */
...
const fRes = await fetch(`/api/featured?exclude=${encodeURIComponent(product.id)}&limit=8`, {
cache: "no-store",
});
let fData: Product[] = [];
if (fRes.ok) {
fData = (await fRes.json()) as Product[];
}

if (alive && Array.isArray(fData) && fData.length > 0) {
setRelated(fData.slice(0, 8));
return;
}

// 2) Fallback: same category (old behavior)
...
if (alive) setRelated(Array.isArray(arr) ? arr.slice(0, 8) : []);
} catch {
if (alive) setRelated([]);
} finally {
...
{/* Optional tiny fallback UI if both featured & category are empty */}
{related.length === 0 && !relLoading && (
<div className="mt-10 text-sm text-slate-400">No suggestions available.</div>
)}
</>
);
}
